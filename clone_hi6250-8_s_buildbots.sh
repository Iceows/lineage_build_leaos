...
VENDOR_COMMON_DEVICES="itsvixano-dev/proprietary_vendor_huawei.git"
VENDOR_HUAWEI_PATH="vendor/huawei/"


# Remove all
rm -rf "$DEVICE_COMMON_PATH"
rm -rf "$VENDOR_HUAWEI_PATH/prague"
rm -rf "$VENDOR_HUAWEI_PATH/hi6250-8-common" 
rm -rf "$KERNEL_PATH"
rm -rf "$HARDWARE_PATH"
...
git clone "$GITLAB""$VENDOR_COMMON_DEVICES" "$VENDOR_HUAWEI_PATH" -b "$BRANCH"
